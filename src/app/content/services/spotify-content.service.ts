...
import { combineLatest, EMPTY, expand, from, map, mergeMap, Observable, reduce } from 'rxjs';
...
export class SpotifyContentService {
  ...
  loadPlaylistTracks(playlistId: string): Observable<Track[]> {
    return this.spotifyService.getPlaylistTracks(playlistId)
      .pipe(
        expand(response => response.next ? this.spotifyService.getPlaylistTracks(playlistId, response.offset + 100) : EMPTY),
  ...
  loadArtistTracks(artistId: string) {
    return this.spotifyService.getArtistAlbums(artistId)
      .pipe(
        mergeMap(response => from(response.items as any[])),
        mergeMap(album => this.spotifyService.getAlbumTracks(album.id)
          .pipe(
            expand(response => response.next ? this.spotifyService.getAlbumTracks(album.id, response.offset + 100) : EMPTY),
            map(response => response.items.map((item: any): Track => {
              return {
                song: item.name,
                album: album.name,
  ...
  private loadSavedAlbums(): Observable<SpotifyContent[]> {
    return this.spotifyService.getSavedAlbums()
      .pipe(
        expand(response => response.next ? this.spotifyService.getSavedAlbums(response.offset + 50) : EMPTY),
        map(response => response.items.map((item: any): SpotifyContent => {
          return {
            id: item.album.id,
  ...
  private loadCurrentUserPlaylists(): Observable<SpotifyContent[]> {
    return this.spotifyService.getCurrentUserPlaylists()
      .pipe(
        expand(response => response.next ? this.spotifyService.getCurrentUserPlaylists(response.offset + 50) : EMPTY),
        map(response => response.items.map((playlist: SpotifyPlaylist): SpotifyContent => {
          return {
            id: playlist.id,
  ...
  private loadFollowedArtists(): Observable<SpotifyContent[]> {
    return this.spotifyService.getFollowedArtists()
      .pipe(
        expand(response => response.artists.next ? this.spotifyService.getFollowedArtists(response.artists.cursors.after) : EMPTY),
        map(response => response.artists.items.map((artist: any): SpotifyContent => {
          return {
            id: artist.id,
